...
public class Particle extends Entity {
	...
	private boolean rotates = false, affectedByGravity = false, shrinks = false, timed = false, fading = false, isColliding = false, isSlowing = false;
	private int maxTime = 0, timeLeft = 0;
	private float gravity = 1.0f, rotationSpeed = 5.0f, gravityDirection = (float) (Math.PI / 2.0f), maxSpeed = 1.0f;
		...
		if (timed) {
			...
			if (timeLeft <= 0) {
				...
				if (isSlowing) {
					...
					setRadialVelocity(maxSpeed * timeLeft / maxTime, getRadialVelocity().y);
	...
	public void setSlowing(boolean isSlowing) {
